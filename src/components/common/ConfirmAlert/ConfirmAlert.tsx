...
function ConfirmAlert(props: Props) {
    ...
    return (
        <Modal title={props.title} open={props.open} closeModal={props.closeModal}>
            <TextField label={"Fyll ut boksen under for å bekrefte"}
                       ref={textRef}
                       isValid={!inputError}
                       placeholder={props.confirmText}
            />
            <div className={styles.buttonRow}>
                <Button color={"danger"} onClick={validateAndConfirm}>Bekreft</Button>
                <Button variant={"outline"} onClick={props.closeModal}>Avbryt</Button>
            </div>
        </Modal>
    )
}
